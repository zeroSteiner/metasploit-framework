...
class MetasploitModule < Msf::Auxiliary
  ...
  def run_host(ip)
    ...
    buffer = samr_con.samr.samr_query_information_domain(
      domain_handle: samr_con.domain_handle,
      info_class: 1
    )
    $stderr.puts "[ 1] buffer = #{buffer.snapshot.inspect}"

    buffer = samr_con.samr.samr_query_information_domain(
      domain_handle: samr_con.domain_handle,
      info_class: 2
    )
    $stderr.puts "[ 2] buffer = #{buffer.snapshot.inspect}"

    buffer = samr_con.samr.samr_query_information_domain(
      domain_handle: samr_con.domain_handle,
      info_class: 3
    )
    $stderr.puts "[ 3] buffer = #{buffer.snapshot.inspect}"

    buffer = samr_con.samr.samr_query_information_domain(
      domain_handle: samr_con.domain_handle,
      info_class: 4
    )
    $stderr.puts "[ 4] buffer = #{buffer.snapshot.inspect}"

    buffer = samr_con.samr.samr_query_information_domain(
      domain_handle: samr_con.domain_handle,
      info_class: 7
    )
    $stderr.puts "[ 7] buffer = #{buffer.snapshot.inspect}"

    buffer = samr_con.samr.samr_query_information_domain(
      domain_handle: samr_con.domain_handle,
      info_class: 12
    )
    $stderr.puts "[12] buffer = #{buffer.snapshot.inspect}"
    ...
    print_good("#{samr_con.domain_name} [ #{users.values.map { |name| name.encode('UTF-8') }.join(', ') } ] ( LockoutTries=0 PasswordMin=7 )")
    samr_con.samr.close_handle(samr_con.domain_handle)
  ensure
